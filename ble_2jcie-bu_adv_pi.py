...
#SENSOR_MAC_NAME = {'CA:43:F0:B6:24:95': 'SU01'} # SU01
SENSOR_MAC_NAME = {'F9:E9:A4:FB:96:C9': 'SU00'} # No number (SU00)
#SENSOR_MAC_NAME = {'CA:43:F0:B6:24:95': 'SU01', # SU01
#                   'F9:E9:A4:FB:96:C9': 'SU00'} # No number (SU00)
...
def advcallback(dev, advdata):
    ...
    if ( dev.name == 'Rbt' ) :
        if ( 0x02D5 in advdata.manufacturer_data.keys() ): #0x02D5=companyID
            ...
            if dev.address in SENSOR_MAC_NAME.keys():
                _sensor_name = SENSOR_MAC_NAME[dev.address]
            else:
                # _sensor_name = re.sub(':', '', dev.address)
                _sensor_name = dev.address
